...
contract Crowdsale is Context, ReentrancyGuard, Ownable {
	...
	uint256 public maxWeiAmountForHolder = 1000 ether;
	uint256 public maxTotalWeiAmount = 10000 ether;
	...
	function buyTokens(address beneficiary) public payable nonReentrant {
		require(startTime != 0, "Presale: Not started yet.");
		require(startTime <= block.timestamp, "Presale not started yet!");
		...
		virtual
	{
		...
		// require(weiAmount >= 0.1 ether, "Presale: weiAmount must be bigger 0.1 ETHW!");
	...
	function _postValidatePurchase()
		...
		view
	{
		require(_holdersWeiRaised[msg.sender] <= maxWeiAmountForHolder, "Presale: weiAmount must be less than max amount!");
		this;
	...
	{
		require(enableTransferToken, "Claim disabled!");
	...
	function _getClaimableTokenAmount(address account) public view returns (uint256)
	{
		...
		return totalClaimableAmount - _holderClaimedAmount[account];
	}
